 ...
 * AbstractActionHandler class and the MessageHandlerCommand interface. The functioning crux
 ...
 * <ul>
 * <li><b>{@link #init(ServletContext, HttpServletRequest)}</b> - Override this
 * method to ensure that the base datasource name is set.</li>
 * <li><b>
 * {@link #processRequest(HttpServletRequest, HttpServletResponse, String)}</b>
 * - Override as the driver only if there are additional commands to process
 * other than "prepare" and "generate".</li>
 * <li><b>{@link #receiveClientData()}</b> - Implement to gather data from the
 * request object in order to build selection criteria.</li>
 * <li><b>{@link #initializeReport()}</b> - Implement to prepare the environment
 * to for report generation. See method documentation.</li>
 ...
 * 
 */
public abstract class AbstractReportAction extends AbstractActionHandler
        implements ICommand {
     ...
     */
    public AbstractReportAction(Context _context, Request _request)
            throws SystemException {
        this();
    ...
    protected void init(Context _context, Request _request)
        ...
        RMT2SessionBean userSession = (RMT2SessionBean) this.getSession()
        ...
        temp = System.getProperty(ConfigConstants.PROPNAME_IMG_DIR);
        this.imageDirPath = this.session.getContext().getRealPath(temp);
        this.imageDirPath += "\\";

        // Get path to xslt files. The results should be the path that is on the
        // classpath
        this.xslPath = System.getProperty(ConfigConstants.PROPNAME_XSLT_PATH);

        // Get Servlet context
        this.webAppCtx = RMT2WebUtility.getWebAppContext(_request);
        // Get command
        ...
        this.reportName += System
                .getProperty(ConfigConstants.PROPNAME_RPT_FILE_EXT);
        // Set the polling service that will trigger the action handler to
        // generate the report.
        // This could be overidden.
        this.pollService = "unsecureRequestProcessor/"
                + (String) parsedRequest.get(0) + "."
                + (String) parsedRequest.get(1);
    ...
    protected String setupReportLayout() throws SystemException {
        ...
        this.xmlFileName = this.getUserWorkArea() + "\\" + this.reportId
                + ".xml";
        this.outFileName = this.getUserWorkArea() + "\\" + this.reportId
                + ".fo";
        try {
            // Attempt to locate Report layout using classpath instead of the
            // file system
            InputStream is = RMT2File.getFileInputStream(origRptPath);
            if (is == null) {
                this.msg = "Report generation failed.   Unable to locate report layout referenced as "
                        + origRptPath;
            ...
            fileData = RMT2String.replaceAll2(fileData, this.imageDirPath,
                    AbstractReportAction.IMAGE_PATH_PLACEHOLDER);
    ...
    public void processRequest(Request request, Response response,
        ...
        if (this.command
                .equalsIgnoreCase(AbstractReportAction.COMMAND_GENERATE)) {
            this.run();
        }
    }
    ...
    public void buildSearchCriteria() throws ActionCommandException {
        ...
        this.getSession()
                .setAttribute(RMT2ServletConst.REPORT_BEAN, this.query);
     ...
     * @throws ActionHandlerException
     */
    public void run() throws ActionCommandException {
        this.query = (RMT2TagQueryBean) this.getSession().getAttribute(
                RMT2ServletConst.REPORT_BEAN);
    ...
    private String buildPollingServiceUrl() {
        String url = null;
        try {
            url = System.getProperty(ConfigConstants.PROPNAME_POLL_PAGE);
     ...
     * @throws ActionHandlerException
     */
    protected String fetchHostCompany(Object dao) throws ActionCommandException {
        if (dao == null) {
    ...
    protected String createWellFormedData(String data) {
        return DataSourceConst.XML_TAG_START + data
                + DataSourceConst.XML_TAG_END;
    }
     ...
     * <li>Fetch and combine the report data from all mandatory sources into one
     * XML data stream.</li>
     ...
     * @throws ActionHandlerException
     */
    protected abstract String initializeReport() throws ActionCommandException;
